...
SESSION 36 ✅ — readability/accessibility pass (typography floor, WCAG-AA contrast, 48px tap targets) + warm-family feature cards + SVG bottom-nav icons — prep for first real-device handoff to 35-55yo users
  ...
  - TAP TARGETS (48px hard constraint actually enforced): home search bar h-10→h-12; hero back/heart 44→48; step-TTS 40→48; "Cooking band karo" 32→48; TTSButton + WhatsAppShare h-10→h-12; ChatWindow send 40→48; Profile family stepper 36→48 + unit toggles 40→48; search chips 40→48 + clear ✕ given 40px hit area (input pr-12); IngredientChips add-chip 36→48, remove ✕ 16→28px (RESIDUAL: 48px would balloon chips — whole chip is also tappable); StarRatingInteractive taps md 28→44/sm 20→36 (RESIDUAL: inline row, full 48 would overflow); PortionSelector trigger min-h-48.
  - WARM CARDS (kills the purple-gradient AI look): feature grid now 160deg warm-family gradients w/ dark bottom stop under the labels — Fridge saffron #E8640C→#BF4E06, Chef Arti terracotta #C4621E→#8C3D0F, Bacha Hua green #2D6A4F→#1E4A36, Aaj ki Thali brick #B23A2E→#7E2A20, Surprise marigold #D97706→#B45309; titles 13→15, subtitles white/80 10 → white/90 12, height 100→108.
  - SVG NAV ICONS: BottomNav emoji (🏠🔍📷👤 — inconsistent colored/greyscale across Android OEMs) → 4 inline stroke SVGs colored by active state (#E8640C / #806244).
  - EMOJI FIX: lib/emoji.ts haldi/turmeric ⭐ → 🟡 (star read as rating, not turmeric).
  - NOT FIXED (deliberate): PortionSlider.tsx (zero importers, legacy — superseded by PortionSelector); admin pages (internal); large bold CTAs on gradient light-end are AA-large-pass (3:1) not AA-normal; sign-in #C4B8A8 footnote (dark bg, passes).
  - VERIFY: tsc clean, next build --webpack clean (PWA sw.js regenerated). Browser-verified at 390px on local dev (home/search/recipe): gradients, SVG nav, 14px samagri ("Lal mirch powder" fits — earlier clipping was screenshot artifact; break-words added as safety). NOT phone-tested — this pass is PREP for the real-device test, which remains open task #1.
