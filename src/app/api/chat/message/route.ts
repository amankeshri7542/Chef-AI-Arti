...
export async function POST(req: NextRequest) {
  ...
  // Whole-route guard: any throw below (body parse, Supabase, Redis session,
  // RAG, prompt file read, session writes) returns a Hinglish 500 instead of
  // an unhandled crash. The GPT call keeps its own dedicated try/catch.
  try {
    ...
    const session: ChatSession = (await getChatSession(userId)) ?? {
      compressed_memory: {
